...
public class BayeuxServerImpl extends AbstractLifeCycle implements BayeuxServer, Dumpable {
            ...
            if (channel == null) {
                ...
                try {
                    ...
                    for (BayeuxServerListener listener : _listeners) {
                        if (listener instanceof Initializer) {
    ...
    public Mutable newMessage(ServerMessage tocopy) {
        Mutable mutable = newMessage();
    ...
    public Mutable handle(ServerSessionImpl session, Mutable message) {
        if (_logger.isDebugEnabled()) {
    ...
    protected void doPublish(ServerSessionImpl from, ServerChannelImpl to, final Mutable mutable) {
    ...
    public Mutable extendReply(ServerSessionImpl from, ServerSessionImpl to, Mutable reply) {
    ...
    protected boolean extendRecv(ServerSession from, Mutable message) {
    ...
    protected Mutable createReply(Mutable message) {
        Mutable reply = newMessage();
    ...
    abstract class HandlerListener implements ServerChannelListener {
        ...
        public abstract void onMessage(final ServerSessionImpl from, final Mutable message);
    }
