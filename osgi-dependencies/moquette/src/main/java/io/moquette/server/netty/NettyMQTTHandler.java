...
public class NettyMQTTHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object message) {
        ...
        try {
            switch (msg.getMessageType()) {
                case CONNECT:
                    m_processor.processConnect(new ServerChannel(ctx), (ConnectMessage) msg);
                    break;
                case SUBSCRIBE:
                    m_processor.processSubscribe(new ServerChannel(ctx), (SubscribeMessage) msg);
                    break;
                case UNSUBSCRIBE:
                    m_processor.processUnsubscribe(new ServerChannel(ctx), (UnsubscribeMessage) msg);
                    break;
                case PUBLISH:
                    m_processor.processPublish(new ServerChannel(ctx), (PublishMessage) msg);
                    break;
                case PUBREC:
                    m_processor.processPubRec(new ServerChannel(ctx), (PubRecMessage) msg);
                    break;
                case PUBCOMP:
                    m_processor.processPubComp(new ServerChannel(ctx), (PubCompMessage) msg);
                    break;
                case PUBREL:
                    m_processor.processPubRel(new ServerChannel(ctx), (PubRelMessage) msg);
                    break;
                case DISCONNECT:
                    m_processor.processDisconnect(new ServerChannel(ctx));
                    break;
                case PUBACK:
                    m_processor.processPubAck(new ServerChannel(ctx), (PubAckMessage) msg);
                    break;
                case PINGREQ:
    ...
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        ...
        if (clientID != null && !clientID.isEmpty()) {
            ...
            m_processor.processConnectionLost(clientID, stolen, new ServerChannel(ctx));
        }
        ctx.close();
    }
